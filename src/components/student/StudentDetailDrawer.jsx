...
import { X, TrendingUp, Tag, Zap, MessageSquare, History } from "lucide-react";
...
/* ── Week timeline item ─────────────────────────────── */
const WeekTimelineItem = ({ entry, isLast, ratingRules }) => {
  const dot = RATING_DOT[entry.rating] || "bg-gray-200";
  const ordinal =
    ratingRules.find((rule) => rule.label === entry.rating)?.value ?? "—";
  return (
    <div className="flex flex-col items-center gap-2 min-w-[100px]">
      <div className="relative flex items-center justify-center w-full">
        ...
        <div
          className={`w-9 h-9 rounded-lg ${dot} flex items-center justify-center text-white font-semibold text-sm z-10`}
      ...
      <div className="text-center">
        ...
        <p className="text-[11px] font-medium text-gray-700 truncate max-w-[88px]">
          {entry.rating}
        </p>
      </div>
    </div>
                    ...
                    <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                      {studentEntries.map((entry, i) => (
                        <WeekTimelineItem
                          key={entry.id}
                          entry={entry}
                          isLast={i === studentEntries.length - 1}
              ...
              <DrawerSection icon={Tag} title="Student Tags">
                <div className="flex flex-wrap gap-2">
                  {student.tags?.map((tag, i) => (
                    <span
                      key={i}
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 text-indigo-600 rounded-lg text-xs font-medium border border-indigo-100"
                    >
                      {tag}
                      <X
                        size={11}
                      ...
                      .map((entry, i) => (
                        ...
                        >
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <span
                              className={`badge-pill ${RATING_STYLES[entry.rating] || "bg-gray-100 text-gray-500"}`}
                            >
                              {entry.rating}
                            </span>
                          </div>
