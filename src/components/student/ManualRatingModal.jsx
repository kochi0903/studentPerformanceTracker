...
const ManualRatingModal = ({
  ...
  const ratingRules = normalizeRatingRules(config);
  const RATING_OPTIONS = ratingRules
    ...
    .map((rule) => {
      ...
      return {
        value: rule.value,
        label: rule.label,
        btn: classes[colorGroup].btn,
        active: classes[colorGroup].active,
        dot: classes[colorGroup].dot,
      };
    });
  ...
  const handleSubmit = async () => {
    ...
    setError("");
    try {
      const ratingLabel = RATING_OPTIONS.find(
        (r) => r.value === selectedRating,
      )?.label;
      const ratingData = {
        rating: ratingLabel,
        notes: notes.trim(),
      ...
      const entry = await studentService.addWeeklyEntry(
        batchId,
        student.id,
        selectedWeek,
        ratingData,
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {RATING_OPTIONS.map((opt) => (
              ...
              >
                <span
                  className={`w-2 h-2 rounded-full flex-shrink-0 ${selectedRating === opt.value ? "bg-white" : opt.dot}`}
                />
                {opt.label}
              </button>
            ))}
