...
const StudentList = () => {
  ...
  const availableWeeks = useMemo(() => {
    const fromEntries = [...new Set(entries.map((e) => e.week))].filter(
      Boolean,
    );
  ...
  const studentStats = useMemo(
    ...
    [students, entries, viewingWeek, activeBatch, ratingRules],
  );
  ...
  const filteredStudents = useMemo(
    () =>
      studentStats.filter((s) => {
        ...
        const matchesTab =
          currentTab === "All" || s.currentRating === currentTab;
  ...
  const stats = useMemo(
    () => ({
      ...
      stars: studentStats.filter((s) => s.flags.includes("Star Student"))
        .length,
      improving: studentStats.filter((s) => s.trend === "Improving").length,
      support: studentStats.filter((s) => s.flags.includes("Need Support"))
        .length,
    }),
  ...
  return (
    <div className="p-6 md:p-8 max-w-[1400px] mx-auto w-full">
      ...
      <ConfirmationModal
        ...
        message={`Remove ${studentToDelete?.name}?`}
        confirmLabel="Delete"
        type="danger"
      ...
      <ConfirmationModal
        ...
        message={`Permanently remove ${selectedIds.length} students. This cannot be undone.`}
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-5 mb-7">
        <div>
          ...
          <nav className="flex items-center gap-1.5 text-[11px] font-medium text-gray-400">
            <span>Dashboard</span>
            <span>·</span>
            <span className="text-gray-700">{activeBatch.name}</span>
            <span>·</span>
            <span>Students</span>
          </nav>
        </div>
          ...
          <div className="flex items-center gap-5 w-full lg:w-auto overflow-x-auto scrollbar-hide">
            {TABS.map((tab) => {
              const count =
                tab === "All"
                  ? stats.total
                  : studentStats.filter((s) => s.currentRating === tab).length;
              return (
                <button
                  key={tab}
              ...
              >
                {availableWeeks.map((w) => (
                  <option key={w} value={w}>
                    Week {w}
                    {w === activeBatch.currentWeek ? " (current)" : ""}
                  </option>
                ))}
              </select>
            </div>
            ...
            <AnimatePresence mode="popLayout">
              {filteredStudents.map((student) => (
                ...
                >
                  ...
                  <div className="w-36">
                    <span
                      className={`badge-pill ${ratingBadge(student.currentRating)}`}
                    >
            ...
            <div className="bg-gray-900 text-white px-5 py-3.5 rounded-xl shadow-2xl flex items-center justify-between gap-4 border border-white/10">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3 py-1.5 text-xs font-medium text-gray-400 hover:text-white transition-colors flex items-center gap-1.5 cursor-pointer"
                >
                  <X size={13} /> Clear
                </button>
                <button
                  onClick={() => setIsBulkDeleteOpen(true)}
                  className="flex items-center gap-1.5 px-4 py-1.5 bg-red-600 hover:bg-red-700 text-white rounded-lg text-xs font-medium transition-colors cursor-pointer"
                >
