...
const LiveSession = () => {
  ...
  const WHEEL_POP_RATINGS = useMemo(() => {
    const ordered = [...ratingRules].sort((a, b) => a.value - b.value);
    const topValue = ordered[ordered.length - 1]?.value || 4;
    return ordered
      .filter((rule) => rule.value >= topValue - 1)
      .map((rule) => rule.label);
  }, [ratingRules]);
    ...
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-[#0a0a0a] text-center p-6">
        ...
        <p className="text-slate-400 max-w-md text-lg font-medium leading-relaxed mb-12">
          All students in{" "}
          <span className="text-white">{activeBatch?.name}</span> have been
          rated for Week {activeBatch?.currentWeek}.
        </p>
        <button
    ...
    <div className="h-screen w-full bg-[#0a0a0a] flex flex-col items-center justify-center overflow-hidden relative font-sans">
      ...
      <div className="absolute top-10 left-10 flex items-center gap-6 z-20">
        ...
        <div className="px-6 py-3 bg-blue-500/10 border border-blue-500/20 rounded-2xl backdrop-blur-xl">
          <p className="text-[10px] font-black text-blue-500/40 uppercase tracking-[0.2em] mb-1">
            Week
          </p>
          <p className="text-lg font-black text-blue-400 tracking-tight">
            {activeBatch?.currentWeek}
          </p>
        </div>
      </div>
        ...
        <div className="w-12 h-12 rounded-full border-2 border-white/10 flex items-center justify-center relative">
          <svg className="w-full h-full -rotate-90">
            ...
            <circle
              ...
              strokeDashoffset={
                125.6 * (eligibleStudents.length / students.length)
              }
            ...
            <button
              ...
              className={`w-16 h-16 rounded-full flex items-center justify-center transition-all ${
                isSpinning
                  ...
                  : "bg-blue-500 text-white hover:scale-110 active:scale-95 cursor-pointer"
              }`}
            >
              <RotateCw
                size={32}
        ...
        >
          <div className="flex items-center gap-3 px-6 py-3 bg-white/5 border border-white/10 rounded-2xl backdrop-blur-xl">
            ...
            <p className="text-xs font-bold text-white/60 tracking-wider">
              Press{" "}
              <span className="text-white px-1.5 py-0.5 bg-white/10 rounded mx-1">
                SPACE
              </span>{" "}
              to Spin
            </p>
          ...
          >
            <div className="max-w-3xl w-full flex flex-col items-center text-center">
              ...
              >
                ...
                <h2 className="text-[10px] font-black text-blue-500 uppercase tracking-[0.5em] mb-4">
                  Presentation By
                </h2>
                <h3 className="text-7xl font-black text-white tracking-tighter uppercase">
                  {winner.name}
                </h3>
              </motion.div>
                  ...
                  >
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-12 mb-12">
                      {RATING_LEVELS.map((level) => {
                        ...
                        return (
                          ...
                          >
                            <div
                              className={`w-14 h-14 rounded-2xl flex items-center justify-center text-white shadow-xl ${level.color}`}
                            >
                              <Icon size={24} />
                            </div>
                    ...
                    <div className="flex flex-col items-center gap-8">
                      ...
                      >
                        <Star
                          size={20}
                          fill={starMoment ? "currentColor" : "none"}
                        />
                        <span className="uppercase tracking-widest text-xs">
                          Flag as Star Moment (S)
                        </span>
                      </button>
                      ...
                      <div className="flex items-center gap-3 text-white/20">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-[0.2em]">
                          Press key 1-4 to rate instantly
                        </p>
                      </div>
                    </div>
                  </motion.div>
