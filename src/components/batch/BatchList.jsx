  ...
  const filteredBatches = useMemo(
    () =>
      batches.filter((b) => {
        ...
        const matchesTab =
          currentTab === "All" || (currentTab === "Active" && !b.isArchived);
            ...
            <AnimatePresence mode="popLayout">
              {filteredBatches.map((batch) => (
                ...
                >
                  ...
                  <div className="w-28">
                    <span
                      className={`badge-pill ${batch.isArchived ? "bg-gray-100 text-gray-500" : "badge-outstanding"}`}
                    >
                  ...
                  <div className="w-28">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-xs font-medium text-gray-600">
                        Week {batch.currentWeek}
                      </span>
                    </div>
                  </div>
                  <div
                    className="w-40 text-right flex items-center justify-end gap-1.5"
                    onClick={(e) => e.stopPropagation()}
                  >
                    <button
                      onClick={() => navigate(`/attendance/${batch.id}`)}
                      title="Attendance"
                      className="p-2 text-emerald-500 hover:bg-emerald-50 rounded-lg transition-colors duration-150 cursor-pointer"
                    >
